...
const CarouselGallery = ({ galleyItems }) => {
  // const galleyItems = [
  //   {
  ...
  return (
    <GlideGallery>
      <GlideGallery.LargeItems className="shadow rounded mb-3">
        {galleyItems.map((item, index) => (
          <GlideGallery.Item key={`gallery.large.${index}`}>
            <img alt="detail" src={item.large} className="responsive border rounded img-fluid w-100" />
          </GlideGallery.Item>
        ))}
